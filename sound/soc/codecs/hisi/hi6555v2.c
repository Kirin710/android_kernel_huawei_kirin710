...
static int switch_hs_gpio(int hs_cmd)
{
	...
	if (priv == NULL) {
		HI6555V2_LOGE("priv is null pointer\n");
		return -EFAULT;
	}

	if (!gpio_is_valid(priv->hs_en_gpio)) {
		HI6555V2_LOGE("Failed to get the headset enable gpio\n");
		return -EFAULT;
	}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ret != 0)
		HI6555V2_LOGE("switch headset enable gpio failed\n");
...
static void hi6555v2_power_codec(struct snd_soc_codec *codec, int on)
{
	...
	if (on) {
		...
		hi6555v2_set_reg_bits(CODEC_ANA_RW12_REG, BIT(CODEC_IDEL_TONE_CTRL_OFFSET));
#endif
		hi6555v2_set_reg_bits(CODEC_ANA_RW3_REG,
			BIT(CODEC_MUTE_ADCL_OFFSET) | BIT(CODEC_MUTE_ADCR_OFFSET) | BIT(CODEC_MUTE_ADC3_OFFSET));
		hi6555v2_set_reg_bits(CODEC_ANA_RW22_REG,
			BIT(CODEC_STB_CTRL_SEC_OFFSET) | BIT(CODEC_STB_CTRL_OFFSET));
		hi6555v2_set_reg_bits(CODEC_ANA_RW23_REG,
...
static void hi6555v2_enable_hsd(struct snd_soc_codec *codec)
{
	IN_FUNCTION;

	hi6555v2_reg_write(codec, CODEC_ANA_RW31_REG, 0x1);

	OUT_FUNCTION;
}

static void hi6555v2_ibias_hsmicbias_enable(struct snd_soc_codec *codec, bool enable)
{
	struct hi6555v2_priv *priv = snd_soc_codec_get_drvdata(codec);
			...
			struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		hi6xxx_mbhc_set_micbias(priv->mbhc, true);
		msleep(1);
		...
		break;
	case SND_SOC_DAPM_POST_PMD:
		hi6xxx_mbhc_set_micbias(priv->mbhc, false);
		break;
	default:
		HI6555V2_LOGE("power mode event err : %d\n", event);
		break;
			...
			struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		hi6555v2_set_reg_bits(R_CODEC_DMA_SEL, BIT(DMIC_SIF_SEL));
		hi6555v2_set_reg_bits(CODEC_CLK_EN1_REG, BIT(DMIC_CLKEN_OFFSET));
		break;
	case SND_SOC_DAPM_POST_PMD:
		hi6555v2_clr_reg_bits(CODEC_CLK_EN1_REG, BIT(DMIC_CLKEN_OFFSET));
		hi6555v2_clr_reg_bits(R_CODEC_DMA_SEL, BIT(DMIC_SIF_SEL));
		break;
	default:
		HI6555V2_LOGE("power mode event err: %d", event);
		break;
	}
...
static int _pmu_resource_init(struct snd_soc_codec *codec)
{
	...
	if (of_property_read_bool(np, "not_support_ldo8")) {
		...
		HI6555V2_LOGI("pmu resource not support ldo8, pmu resource init ok\n");
		return 0;
	}
...
static void _pmu_resource_deinit(struct snd_soc_codec *codec)
{
	...
	if (priv->is_not_support_ldo8) {
		HI6555V2_LOGI("not support ldo8, pmu resource deinit ok\n");
		return;
	}
...
static void hi6555v2_hs_gpio_init(struct snd_soc_codec *codec)
{
	unsigned int val;
	struct hi6555v2_priv *priv = NULL;
	...
	if (codec == NULL) {
		HI6555V2_LOGE("codec is null pointer\n");
		return;
	}
	...
	if (node == NULL) {
		HI6555V2_LOGE("node is null pointer\n");
		return;
	}

	if (!of_property_read_u32(node, "hisilicon,hs_en_gpio", &val)) {
		HI6555V2_LOGI("headset enable gpio is %d\n", val);
	} else {
		HI6555V2_LOGI("headset enable gpio is not support\n");
		return;
	}

	if (!gpio_is_valid(val)) {
		HI6555V2_LOGE("headset enable gpio is invalid\n");
		return;
	}

	if (gpio_request(val, "hs_en_gpio") != 0) {
		HI6555V2_LOGE("headset enable gpio request failed\n");
		return;
	}
	...
	if (gpio_direction_output(priv->hs_en_gpio, 1) != 0) {
		...
		priv->hs_en_gpio = ARCH_NR_GPIOS;
		HI6555V2_LOGE("headset enable gpio set output failed\n");
		return;
	}
	priv->hs_en_value = 1;

	HI6555V2_LOGI("headset enable gpio init success\n");
}
...
static int hi6555v2_codec_probe(struct snd_soc_codec *codec)
{
	...
	priv = snd_soc_codec_get_drvdata(codec);
	WARN_ON(!priv);
	ops.set_ibias_hsmicbias = hi6555v2_ibias_hsmicbias_enable;
	ops.enable_hsd = hi6555v2_enable_hsd;
	ret = hi6xxx_mbhc_init(codec, &priv->mbhc, &ops);
	if (ret) {
