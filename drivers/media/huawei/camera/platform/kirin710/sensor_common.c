...
static int hw_sensor_spec_match(const char *dts_str, const char *camera_spec)
{
	...
	if (!chr_underline) {
		...
		if (strncpy_s(custom_spec, sizeof(custom_spec), chr_colon,
			chr_underline - chr_colon > 0 ? chr_underline - chr_colon : 0) != 0)
			return -1;
	}
    ...
    sensor_t *sensor)
{
    ...
    cam_debug("%s huawei,sensor_spec %s, rc %d\n", __func__,
        sensor_info->sensor_spec, rc);
    if (rc < 0 || !(sensor_info->sensor_spec) || *(sensor_info->sensor_spec) == '\0') {
        cam_err("%s no huawei,sensor_spec config\n", __func__);
    } else {
        camera_specs = get_camera_specs();
        if (camera_specs && *camera_specs != '\0') {
            if (hw_sensor_spec_match(sensor_info->sensor_spec, camera_specs) < 0) {
                rc = -1;
                cam_err("%s failed %d. the current board custom camera_spec "
                    "is %s, so can't support %s\n",
                    __func__, __LINE__, camera_specs, sensor_info->sensor_spec);
                goto fail;
            }
        }
    }
