...
static int left_data_parse_thread(void *p)
{
	...
	while (!runtime->kthread_should_stop) {
		...
		if (ret == -ETIME)
			AUDIO_LOGE("proc sema down_int err -ETIME");

		ret = down_interruptible(&runtime->hook_stop_sema);
		if (ret == -ETIME)
			AUDIO_LOGE("stop sema down_int err -ETIME");
...
static int init_para(unsigned int codec_type)
{
	...
	if (codec_type == HI64XX_CODEC_TYPE_6403) {
		...
		AUDIO_LOGE("do not support codec_type %d", codec_type);
		WARN_ON(true);
		return -EINVAL;
	}
	...
	const char *path)
{
	...
	if (IS_ERR(fp)) {
		if (priv->open_file_failed_cnt < PRINT_MAX_CNT) {
			AUDIO_LOGE("open file fail: %s", path);
	...
	if (sys_chown((const char __user *)path, ROOT_UID, SYSTEM_GID))
		AUDIO_LOGE("chown %s failed", path);
...
int hi64xx_set_hook_path(const struct hi64xx_param_io_buf *param)
{
	...
	AUDIO_LOGI("path:%s", priv->hook_path);
	...
	unsigned int codec_type)
{
	...
	if (hook_priv == NULL) {
		AUDIO_LOGE("hi64xxdsp hook malloc failed");
		return -ENOMEM;
	}
