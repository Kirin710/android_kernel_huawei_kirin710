...
/* Search for the corresponding modid through the level-2 reset cause of the DMSS */
unsigned int get_sub_reason_modid(u32 master_reason, u32 sub_reason)
{
	u32 i;
	static u32 max_einfo = ARRAY_SIZE(g_einfo);

	for (i = 0; i < max_einfo; i++)
		if (master_reason == g_einfo[i].e_exce_type && sub_reason == g_einfo[i].e_exce_subtype) {
			pr_info("[%s], modid is [0x%x]\n", __func__, g_einfo[i].e_modid);
			return g_einfo[i].e_modid;
		}
...
static int io_resources_init(void)
{
	...
	for (i = 0; i < g_rdr_ap_root->num_reg_regions; i++) {
		if (of_address_to_resource(np, i, &res)) {
			BB_PRINT_ERR("[%s], of_address_to_resource [%u] fail!\n", __func__, i);
			goto timer_ioinit;
		}
...
int save_exception_info(void *arg)
{
	...
	ret = create_exception_dir(exce_dir, dst_dir_str);
	if (ret < 0) {
