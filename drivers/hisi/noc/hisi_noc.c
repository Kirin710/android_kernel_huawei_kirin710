...
int is_noc_node_available(struct noc_node *node)
{
	...
	pr_err("[%s]NOC_POWER_IDLE_REQ = 0x%x;\n", __func__, request);
	pr_err("[%s]NOC_POWER_IDLE_ACK = 0x%x;\n", __func__, ack);
	pr_err("[%s]NOC_POWER_IDLE_STATUS = 0x%x;\n", __func__, status);
	pr_err("[%s]node->pwrack_bit = %u;\n", __func__, node->pwrack_bit);
...
static void hisi_noc_reset_check(u32 modid_num, u32 *temp_modid, unsigned int reset_flag)
{
	if (((modid_num > 0) && (g_giveup_idle_retry > MAX_GIVEUP_IDLE_NUM)) || reset_flag)
...
#ifdef CONFIG_HISI_BB
	if (check_himntn(HIMNTN_NOC_ERROR_REBOOT))
		hisi_noc_reset_check(modid_num, temp_modid, reset_flag);
#endif
