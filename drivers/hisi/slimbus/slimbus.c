	...
	struct slimbus_track_param *params)
{
	...
	if (track != SLIMBUS_TRACK_SOUND_TRIGGER) {
		AUDIO_LOGE("track %d activate is not soundtriger", track);
		return -1;
	}
...
static int32_t slimbus_check_pm(uint32_t track)
{
	...
	if (pdata->pm_runtime_support) {
		if (pdata->track_state[track]) {
			AUDIO_LOGI("track: %d has been configured", track);
		} else {
			ret = pm_runtime_get_sync(pdata->dev);
			if (ret < 0) {
				AUDIO_LOGE("pm resume error, track: %d, ret: %d", track, ret);
	...
	uint32_t track)
{
	...
	if (!pdata->track_state || (track >= pdata->slimbus_track_max)) {
		AUDIO_LOGE("params error, track %d", track);
		return false;
	}
	...
	uint32_t track, struct slimbus_track_param *params)
{
	...
	if (!slimbus_track_params_is_valid(dev_type, track)) {
		AUDIO_LOGE("params error, dev_type %d, track %d", dev_type, track);
		return -EINVAL;
	}
	...
	if (ret) {
		AUDIO_LOGE("slimbus activate track %d fail", track);
		...
		const struct slimbus_track_config *config)
{
	...
	if (!ret)
		...
		AUDIO_LOGE("fail track %d, ret %d", track, ret);
	...
	uint32_t track, struct slimbus_track_param *params)
{
	...
	if (pdata->pm_runtime_support) {
		if (!pdata->track_state[track]) {
			AUDIO_LOGE("track: %d has been removed", track);
			mutex_unlock(&dev->track_mutex);
			return 0;
		}
	}
	...
	AUDIO_LOGI("track: %d trackstate:0x%x portstate:0x%x pm_usage_count:0x%x",
...
struct slimbus_bus_config *slimbus_get_bus_config(void)
{
	return bus_config;
}
	...
	enum slimbus_device device_type, enum platform_type *type)
{
	...
	*type = PLATFORM_PHONE;
	...
	AUDIO_LOGI("platform type: %s device_type:%d slimbusclk: 0x%x, slimbusdata: 0x%x, slimbusclk_cfg: 0x%x slimbusdata_cfg: 0x%x",
		platform_type, device_type, slimbus_devices[device_type]->slimbusclk_offset,
...
{
	int32_t ret = slimbus_drv_init(pd->type, pd->base_addr, pd->asp_reg_base_addr, pd->irq);
	...
	struct slimbus_private_data *pd, uint32_t asppower, uint32_t aspclk)
{
	...
	if (pd->lastframer == SLIMBUS_FRAMER_CODEC) {
		...
		AUDIO_LOGI("switch_framer:%#x +", pdata->lastframer);
	}
...
{
	uint32_t asppower = slimbus_utils_asp_state_get(pdata->sctrl_base_addr, pdata->asp_power_state_offset);
	uint32_t aspclk = slimbus_utils_asp_state_get(pdata->sctrl_base_addr, pdata->asp_clk_state_offset);

	AUDIO_LOGI("portstate: 0x%x trackstate: 0x%x asppower: 0x%x aspclkstate: 0x%x usage_count: 0x%x status: %x clk: %d  ++",
		pdata->portstate, slimbus_trackstate_get(), asppower, aspclk, atomic_read(&(pdata->dev->power.usage_count)),
		pdata->dev->power.runtime_status, clk_get_enable_count(pdata->pmu_audio_clk));
}
