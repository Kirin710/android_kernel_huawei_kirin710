...
bool g_hisee_api_bypassed = false;
bool g_hisee_file_ioctl_ready = false;
...
static ssize_t hisee_ioctl_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (g_hisee_api_bypassed) {
		ret = strncpy_s(buf, HISEE_CMD_NAME_LEN,
		                HISEE_API_BYPASS_MSG, sizeof(HISEE_API_BYPASS_MSG));
