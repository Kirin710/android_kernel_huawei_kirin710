...
int set_favor_small_cap(const void __user *uarg)
{
	struct task_config config;
	struct task_struct *p = NULL;

	if (copy_from_user(&config, uarg, sizeof(struct task_config)))
		return -EFAULT;

	rcu_read_lock();

	p = find_task_by_vpid(config.pid);
	if (!p) {
		rcu_read_unlock();
		return -ESRCH;
	}
...
err:
	...
	case PERF_CTRL_SET_FAVOR_SMALL_CAP:
		return set_favor_small_cap(uarg);
#endif
