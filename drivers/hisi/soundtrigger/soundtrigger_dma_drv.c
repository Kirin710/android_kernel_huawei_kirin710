...
static int32_t dma_start(struct st_fast_status * fast_status)
{
	...
	if (CODEC_HI6403 == dma_drv_info->type) {
	...
	(void)hi64xx_request_pll_resource(HIGH_FREQ_SCENE_FASTTRANS);
	msleep(2);
	...
	if (ret != 0) {
		...
		hi64xx_release_pll_resource(HIGH_FREQ_SCENE_FASTTRANS);
		return ret;
	}
...
static int32_t dma_close(void)
{
	...
	if (dma_drv_info->is_slimbus_enable) {
		...
		err = slimbus_deactivate_track(device_type, track_type, NULL);
		if (err)
...
static int32_t soundtrigger_dma_drv_probe (struct platform_device *pdev)
{
	...
	int32_t  err;
	int32_t  ret;
	unsigned int sochifi_wakeup_upload_chn_num = ONEMIC_CHN;
	...
	ret = of_property_read_u32(dev->of_node, "sochifi_wakeup_upload_chn_num", &sochifi_wakeup_upload_chn_num);
	if (ret < 0) {
		sochifi_wakeup_upload_chn_num = ONEMIC_CHN;
		logw("sochifi_wakeup_upload_chn_num property read failed.\n");
	} else {
		logi("sochifi_wakeup_upload_chn_num: %d, read ok\n", sochifi_wakeup_upload_chn_num);
	}
	...
	soundtrigger_socdsp_pcm_init(sochifi_wakeup_upload_chn_num);
