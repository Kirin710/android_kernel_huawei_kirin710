...
static inline bool check_headset_pluged_in(struct hi64xx_mbhc_priv *priv)
{
	...
	if(check_usb_analog_hs_support() || ana_hs_support_usb_sw()) {
		if(usb_analog_hs_check_headset_pluged_in() == ANA_HS_PLUG_IN ||
			...
			pr_info("ananlog hs is PLUG_IN\n");
			return true;
		} else {
			pr_info("ananlog hs is PLUG_OUT\n");
			return false;
		}
	} else {
...
void hi64xx_plug_in_detect(struct hi64xx_mbhc_priv *priv)
{
	...
	hi64xx_hstype_identify(priv, &headset_type, voltage_value);
	...
	if(check_anc_hs_interface_support()  && priv->hs_status == HISI_JACK_HEADSET) {
		//mask btn irqs while control boost
		hi64xx_irq_mask_btn_irqs(&priv->mbhc_pub);
		priv->anc_hs_plug_status = anc_hs_interface_charge_detect(voltage_value, headset_type);
		hi64xx_irq_unmask_btn_irqs(&priv->mbhc_pub);
	}

	anc_hs_interface_refresh_headset_type(headset_type);
	/* real invert headset */
...
#endif
	ana_hs_refresh_headset_type(headset_type);
		...
		struct hi64xx_mbhc **mbhc)
{
	...
	*mbhc = &priv->mbhc_pub;
	...
	usb_analog_hs_dev_register(&ana_hs_dev, priv);
	ana_hs_codec_dev_register(&ana_hs_dev, priv);
