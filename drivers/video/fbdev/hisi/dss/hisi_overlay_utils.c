...
static void adjust_dst_rect_x(struct hisi_fb_data_type *hisifd, dss_overlay_t *pov_req, dss_rect_t *pdst_rect)
{
	...
	if ((dummy_pixel_num != 0) && (pov_req->rog_width > 0)) {
		dummy_pixel_num = dummy_pixel_num * pov_req->rog_width / (hisifd->panel_info.xres - hisifd->panel_info.dummy_pixel_num);
...
bool is_arsr_post_need_padding(dss_overlay_t *pov_req, struct hisi_panel_info *pinfo, uint32_t ihinc)
{
	...
	if (pinfo != NULL && pinfo->dummy_pixel_num > 0) {
		HISI_FB_DEBUG("dummy_pixel_num %d", pinfo->dummy_pixel_num);
		return false;
	}
	...
	if (pov_req != NULL && pinfo != NULL &&
		(pov_req->res_updt_rect.w > 0) && (pov_req->res_updt_rect.w < pinfo->xres) &&
		(pov_req->res_updt_rect.h > 0) && (pov_req->res_updt_rect.h < pinfo->yres)) {
		ret = true;
	...
	dss_rect_t *out_aligned_rect, bool *rdma_stretch_enable)
{
	...
	dss_rect_ltrb_t aligned_src_rect = {0, 0, 0, 0};
	...
	*rdma_stretch_enable = (stretched_line_num > 0) ? true : false;
	...
	if (layer->need_cap & CAP_AFBCD) {
		...
		// original aligned rect
		aligned_src_rect.left = ALIGN_DOWN((uint32_t)src_rect.x, AFBC_BLOCK_ALIGN);
		aligned_src_rect.right = ALIGN_UP(src_right, AFBC_BLOCK_ALIGN) - 1;
		aligned_src_rect.top = ALIGN_DOWN((uint32_t)src_rect.y, AFBC_BLOCK_ALIGN);
		aligned_src_rect.bottom = ALIGN_UP(src_bottom, AFBC_BLOCK_ALIGN) - 1;
		...
		rdma_oft_pos.rdma_oft_x0 = aligned_src_rect.left / aligned_pixel;
		rdma_oft_pos.rdma_oft_x1 = aligned_src_rect.right / aligned_pixel;
		...
		ret = hisi_dss_rdma_head_check(layer, afbc_header_addr, afbc_header_stride,
			aligned_src_rect, afbc_rect, &afbc_header_start_pos);
		if(ret != 0){
			return -EINVAL;
		}

		//payload
		ret = hisi_dss_rdma_payload_set(layer,&afbc_payload_stride,&afbc_payload_addr,aligned_src_rect);
		if(ret != 0){
			return -EINVAL;
		}
	...
	dss_layer_t *layer, dss_rect_t *aligned_rect, bool rdma_stretch_enable)
{
	...
	transform = layer->transform;
#if defined(CONFIG_HISI_FB_V320) || defined (CONFIG_HISI_FB_V330)
	chn_idx_temp = DSS_RCHN_V1; /*dssv320 v1 surport vscfh*/
#else
...
#if defined(CONFIG_HISI_FB_V320) || defined (CONFIG_HISI_FB_V330)
		(void)i;
		set_reg(mctl_sys_base + MCTL_MOD0_DBG, 0xA8000, 32, 0);
		set_reg(mctl_sys_base + MCTL_MOD1_DBG, 0xA8000, 32, 0);
		set_reg(mctl_sys_base + MCTL_MOD4_DBG, 0xA8000, 32, 0);
		set_reg(mctl_sys_base + MCTL_MOD5_DBG, 0xA8000, 32, 0);
		set_reg(mctl_sys_base + MCTL_MOD6_DBG, 0xA8000, 32, 0);
		set_reg(mctl_sys_base + MCTL_MOD7_DBG, 0xA8000, 32, 0);
		set_reg(mctl_sys_base + MCTL_MOD8_DBG, 0xA8000, 32, 0);
		set_reg(mctl_sys_base + MCTL_MOD10_DBG, 0xA0000, 32, 0);
		set_reg(mctl_sys_base + MCTL_MOD12_DBG, 0xA0000, 32, 0);
		set_reg(mctl_sys_base + MCTL_MOD14_DBG, 0xA0000, 32, 0);
		set_reg(mctl_sys_base + MCTL_MOD15_DBG, 0xA0000, 32, 0);
		set_reg(mctl_sys_base + MCTL_MOD17_DBG, 0xA0F00, 32, 0);
	  ...
	  dss_overlay_block_t *pov_h_block, dss_rect_t *wb_ov_block_rect, int ovl_idx, int ov_h_block_idx)
{
	...
	if (wb_ov_block_rect == NULL &&
		is_arsr_post_need_padding(pov_req, pinfo, ARSR1P_INC_FACTOR)) {
		img_width += ARSR_POST_COLUMN_PADDING_NUM;
		HISI_FB_DEBUG("img_width = %d, padding %d column", img_width, ARSR_POST_COLUMN_PADDING_NUM);
	}
	...
	dss_layer_t *layer, dss_rect_t *wb_ov_block_rect, bool has_base)
{
	...
	ovl->ovl_layer_used[layer_idx] = 1;
#if defined(CONFIG_HISI_FB_V320) || defined (CONFIG_HISI_FB_V330)
	if ((layer->chn_idx == DSS_RCHN_V1) && layer->block_info.arsr2p_left_clip) {
#else
	...
	if (wb_ov_block_rect != NULL) {
		...
		if (is_arsr_post_need_padding(pov_req, &(hisifd->panel_info), ARSR1P_INC_FACTOR)) {
			...
			HISI_FB_DEBUG("dst_rect.x = %d, shift %d column",
				dst_rect.x, ARSR_POST_COLUMN_PADDING_NUM);
		}
	...
	dss_overlay_t *pov_req)
{
	...
	if (hisifd->panel_info.dfr_support) {
		dirty_region_updt->ldi_vrt_ctrl0 = set_bits32(dirty_region_updt->ldi_vrt_ctrl0,
			(hisifd->panel_info.frm_rate_ctrl.current_vfp + v_porch_pading), 32, 0);
		if (hisifd->panel_info.dfr_support && (v_porch_pading > 0) &&
			(hisifd->panel_info.frm_rate_ctrl.status == FRM_UPDT_NEED_DOING))
			HISI_FB_ERR("dirty region update conflict with dfr");
	} else {
		dirty_region_updt->ldi_vrt_ctrl0 = set_bits32(dirty_region_updt->ldi_vrt_ctrl0,
			(pinfo->mipi.vfp + v_porch_pading), 32, 0);
	}
#else
	dirty_region_updt->ldi_vrt_ctrl0 = set_bits32(dirty_region_updt->ldi_vrt_ctrl0,
	...
	dss_wb_layer_t *layer, dss_rect_t aligned_rect, dss_rect_t *ov_block_rect, bool last_block)
{
	...
	#if defined(CONFIG_HISI_FB_970) || defined(CONFIG_HISI_FB_V501) || \
		defined(CONFIG_HISI_FB_V510) || defined(CONFIG_HISI_FB_V600)
		ret = hisi_dss_hfbce_config(hisifd, pov_req, layer, aligned_rect, ov_block, last_block);
		if (ret != 0) {
	...
	if (layer->need_cap & CAP_AFBCE) {
		...
		if (layer->transform & HISI_FB_TRANSFORM_ROT_90) {
			...
			afbc_header_rect.top = ALIGN_DOWN((uint32_t)(layer->dst_rect.y + (ov_block->x - layer->dst_rect.x)),
				AFBC_BLOCK_ALIGN);
	...
	wdma_addr = hisi_calculate_display_addr_wb(mmu_enable, layer, in_rect, ov_block, DSS_ADDR_PLANE0);
	...
	if (layer->transform & HISI_FB_TRANSFORM_ROT_90) {
		wdma->rot_size = set_bits32(wdma->rot_size,
			((uint32_t)DSS_WIDTH(ov_block->w) | ((uint32_t)DSS_HEIGHT(aligned_rect.h) << 16)), 32, 0);

		wdma_buf_width = DSS_HEIGHT(ov_block->h);
		wdma_buf_height = DSS_WIDTH(ov_block->w);

	} else {
		wdma_buf_width = DSS_WIDTH(ov_block->w);
		wdma_buf_height = DSS_HEIGHT(ov_block->h);
	...
	if (is_YUV_SP_420(layer->dst.format)) {
		wdma_addr = hisi_calculate_display_addr_wb(mmu_enable, layer, in_rect, ov_block, DSS_ADDR_PLANE1);
	...
	if (g_debug_ovl_offline_composer) {
		HISI_FB_INFO("dma_oft[x0_y0_x1_y1][%d:%d:%d:%d],rot_size[w_h][%d:%d],"
			...
			ov_block->w, aligned_rect.h, wdma_buf_width, wdma_buf_height);
	}
...
int hisi_overlay_on(struct hisi_fb_data_type *hisifd, bool fastboot_enable)
{
	...
	#if !defined(CONFIG_HISI_FB_V320) && !defined (CONFIG_HISI_FB_V330) && !defined (CONFIG_HISI_FB_V350)
...
void hisi_vactive0_start_isr_handler(struct hisi_fb_data_type *hisifd)
{
	...
	if (g_debug_enable_asynchronous_play) {
		if (hisifd->panel_info.dfr_support == 1) {
			if (is_mipi_cmd_panel(hisifd)) {
				hisifd->asynchronous_vactive0_start_flag = 1;
