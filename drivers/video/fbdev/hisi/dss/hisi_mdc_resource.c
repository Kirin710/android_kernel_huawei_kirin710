	...
	for (i = 0; i < mdc_ops->chan_num; i++) {
		...
		if ((mdc_chn->status == HWC_USED) && (chn_info->hold_flag == MDC_REQUEST)) {
				...
				if ( mdc_chn->drm_used == 1) {
					...
					HISI_FB_DEBUG("hwc drm is using! \n");
					continue;
				}

				hisifd->need_refresh = true;
				//mdc request channel, wait and take off this channel
				HISI_FB_DEBUG("hwc drm is not using! \n");
				wake_up_interruptible_all(&(mdc_ops->refresh_handle_wait));
			#endif
...
int hisi_mdc_chn_request(struct fb_info *info, void __user *argp)
{
	...
	if (primary_hisifd && (primary_hisifd->secure_ctrl.secure_event == DSS_SEC_ENABLE)) {
		HISI_FB_INFO("fb(%d) in TUI mode.", hisifd->index);
		return -EINVAL;
	}
#endif
	mdc_ops = &(hisifd->mdc_ops);
