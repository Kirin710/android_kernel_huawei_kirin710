				...
				struct hisi_panel_info *pinfo)
{
	...
	if (*lane_clock > 2500 || *lane_clock < 80) {
		HISI_FB_ERR("80M<=lane_clock<=2500M, not support %llu M\n",
					*lane_clock);
		*lane_clock = 960;
	}
	...
	*lane_clock = phy_ctrl->rg_pll_fbkdiv * DEFAULT_MIPI_CLK_RATE;
	...
	HISI_FB_INFO("rg_pll_prediv=%d, rg_pll_posdiv=%d\n"
			"rg_pll_fbkdiv=%d, lane_clock = %llu\n",
				...
				struct hisi_panel_info *pinfo)
{
	...
	for (i = 0; i < 2; i++) {
		...
		m_pll_remainder[i] = ((uint64_t)(n_pll * (*lane_clock) *
			1000000UL * 1000UL / DEFAULT_MIPI_CLK_RATE) % 1000) *
			10 / 1000;
	}
	...
	m_pll = n_pll * (*lane_clock) * vco_div * 1000000UL /
			...
			uint32_t *out_ui)
{
	...
	*out_ui = ui;
	...
	clk_t_hs_prepare = ((380 * accuracy +
		pinfo->mipi.clk_t_hs_prepare_adjust * ui) <=
		(950 * accuracy - 8 * ui)) ?
	...
	data_t_hs_prepare =  ((400  * accuracy +
		4 * ui + pinfo->mipi.data_t_hs_prepare_adjust * ui) <=
	...
	data_t_hs_zero = 1450 * accuracy + 10 * ui - data_t_hs_prepare +
		3 * unit_tx_byte_clk_hs +
		pinfo->mipi.data_t_hs_zero_adjust * ui;
	...
	phy_ctrl->data_t_lpx =
		round1(data_t_lpx, unit_tx_byte_clk_hs);
	phy_ctrl->data_t_hs_zero =
		round1(data_t_hs_zero, unit_tx_byte_clk_hs);
...
static void get_dsi_phy_ctrl_v320(struct hisi_fb_data_type *hisifd,
			struct mipi_dsi_phy_ctrl *phy_ctrl,
			uint32_t mipi_dsi_bit_clk)
{
	...
	/*******  clock/data lane parameters config  **********/
	get_dsi_phy_ctrl_lane_para(lane_clock, phy_ctrl, pinfo, &ui);
	...
	// unit of measurement
	unit_tx_byte_clk_hs = 8 * ui;
	if (unit_tx_byte_clk_hs == 0) {
		HISI_FB_ERR("unit_tx_byte_clk_hs is 0");
		return;
	}
	...
	if (pinfo->mipi.lp11_flag == MIPI_SHORT_LP11) {
		...
		phy_ctrl->data_pre_delay = clk_pre_delay_reality +
			phy_ctrl->clk_t_lpx + phy_ctrl->clk_t_hs_prepare +
			clk_t_hs_zero_reality + round1(clk_pre, unit_tx_byte_clk_hs);
	}
	...
	HISI_FB_DEBUG("PHY lane config: rg_vrefsel_vcm=%u,clk_pre_delay=%u\n"
	...
	HISI_FB_DEBUG("clk_lane_lp2hs_time=%u, clk_lane_hs2lp_time=%u\n"
...
static void mipi_phy_init_config_v14(
			struct hisi_fb_data_type *hisifd,
			char __iomem *mipi_dsi_base)
{
	struct hisi_panel_info *pinfo = &(hisifd->panel_info);

	HISI_FB_INFO("mipi phy init v14\n");
	...
	mipi_config_phy_test_code(mipi_dsi_base, 0x00010063,
	...
	if (pinfo->mipi.rg_vrefsel_vcm_adjust != 0) {
		mipi_config_phy_test_code(mipi_dsi_base, 0x00010068,
...
static void mipi_phy_init_config(struct hisi_fb_data_type *hisifd,
			...
			char __iomem *mipi_dsi_base)
{
	...
	mipi_phy_init_config_v14(hisifd, mipi_dsi_base); // normal rate
	udelay(100);
			...
			char __iomem *mipi_dsi_base)
{
	...
	mipi_dsi_bit_clk = 200;
	get_dsi_phy_ctrl_v320(hisifd, &(pinfo->dsi_phy_ctrl),mipi_dsi_bit_clk);
	mipi_phy_init_config(hisifd, mipi_dsi_base, 1);// 400M
	set_reg(mipi_dsi_base + MIPIDSI_PHY_RSTZ_OFFSET, 0x1, 1, 0);
	udelay(100);

	mipi_dsi_bit_clk = pinfo->mipi.dsi_bit_clk_upt;
	get_dsi_phy_ctrl_v320(hisifd, &(pinfo->dsi_phy_ctrl), mipi_dsi_bit_clk);
	mipi_phy_init_config(hisifd, mipi_dsi_base, 2);// normal rate
	udelay(100);

	mipi_config_phy_test_code(mipi_dsi_base, 0x00010014,
		(1 << 4) + (pinfo->dsi_phy_ctrl.rg_pll_pre_div1p << 3) +
	...
	mipi_config_phy_test_code(mipi_dsi_base, 0x0001001F,
			...
			char __iomem *mipi_dsi_base)
{
	if (g_mipi_dphy_version == DPHY_VER_14)
		mipi_phy_pll_start_enhance_config_v14(hisifd, mipi_dsi_base);
	else
		mipi_phy_pll_start_enhance_config_v12(hisifd, mipi_dsi_base);
}
...
void mipi_init(struct hisi_fb_data_type *hisifd, char __iomem *mipi_dsi_base)
{
	...
	if (hisifd->panel_info.mipi.txoff_rxulps_en)
...
REDO:
	...
	mipi_phy_mask_ctrl(mipi_dsi_base, false, hisifd->panel_info.mipi.txoff_rxulps_en);
...
static int mipi_dsi_ulps_exit_for_video(struct hisi_fb_data_type *hisifd,
	char __iomem *mipi_dsi_base)
{
	uint32_t tmp = 0;
	uint32_t cmp_ulpsactivenot_val = 0;
...
static void mipi_reset_dsi(struct hisi_fb_data_type *hisifd)
{
	if (hisifd == NULL) {
		HISI_FB_ERR("hisifd is NULL");
		return;
...
int mipi_dsi_on(struct platform_device *pdev)
{
	...
	if (mipi_dsi_is_phy_special()) {
		 ...
		 * restore pll to open
		 */
		mipi_config_phy_test_code(hisifd->mipi_dsi0_base, 0x00010070, 0x0);
		mipi_config_phy_test_code(hisifd->mipi_dsi0_base, 0x00010069, 0x61);
		mipi_config_phy_test_code(hisifd->mipi_dsi0_base, 0x00010069, 0x1);
	}
	...
	// only used for v320
	if (hisifd->panel_info.mipi.txoff_rxulps_en) {
		mipi_dsi_ulps_cfg(hisifd, false);
		mdelay(1);
		mipi_phy_mask_ctrl(hisifd->mipi_dsi0_base, false, 0);
		mipi_dsi_ulps_exit_for_video(hisifd, hisifd->mipi_dsi0_base);
	}
...
int mipi_dsi_off(struct platform_device *pdev)
{
	...
	if (mipi_dsi_is_phy_special()) {
		...
		*/
		mipi_config_phy_test_code(hisifd->mipi_dsi0_base, 0x00010072, 0x1f);
		mipi_config_phy_test_code(hisifd->mipi_dsi0_base, 0x00010073, 0x0);

		mipi_config_phy_test_code(hisifd->mipi_dsi0_base, 0x00010070, 0x61);
		mipi_config_phy_test_code(hisifd->mipi_dsi0_base, 0x00010072, 0x18);
		mipi_config_phy_test_code(hisifd->mipi_dsi0_base, 0x00010072, 0x1f);

		mipi_config_phy_test_code(hisifd->mipi_dsi0_base, 0x00010063, 0x12);
		mipi_config_phy_test_code(hisifd->mipi_dsi0_base, 0x00010062, 0x2);
		mipi_config_phy_test_code(hisifd->mipi_dsi0_base, 0x00010061, 0x0);
		mipi_config_phy_test_code(hisifd->mipi_dsi0_base, 0x00010060, 0x0);
	}
	...
	if (mipi_dsi_is_phy_special()) {
		 ...
		 * pll open and low
		 */
		mipi_config_phy_test_code(hisifd->mipi_dsi0_base, 0x00010072, 0x7);
		mipi_config_phy_test_code(hisifd->mipi_dsi0_base, 0x00010067, 0x0);
		mipi_config_phy_test_code(hisifd->mipi_dsi0_base, 0x00010068, 0x0);
	}
	...
	char __iomem *mipi_dsi_base, struct mipi_dsi_phy_ctrl* phy_ctrl)
{
	...
	if (g_mipi_dphy_version == DPHY_VER_12) {
		...
		mipi_phy_init_config_v14(hisifd, mipi_dsi_base);
	}
	// set dphy spec parameter
	mipi_config_dphy_spec_parameter(mipi_dsi_base, pinfo);
	...
	char __iomem *mipi_dsi_base, struct mipi_dsi_phy_ctrl* phy_ctrl)
{
	...
	if (g_mipi_dphy_version == DPHY_VER_12) {
		...
		// physical configuration PLL II, M
		mipi_config_phy_test_code(mipi_dsi_base, 0x00010015, pinfo->dsi_phy_ctrl.rg_div);
		// physical configuration PLL I, N
		// mipi_config_phy_test_code(mipi_dsi_base, 0x00010014, (pinfo->dsi_phy_ctrl.rg_pre_div << 3));
	} else {
		mipi_phy_init_config_v14(hisifd, mipi_dsi_base);
	}
	// Configure core's phy parameters
...
int mipi_dsi_bit_clk_upt_isr_handler(struct hisi_fb_data_type *hisifd)
{
	...
	hisifb_get_timestamp(&tv0);
	lane_byte_clk = hisifd->panel_info.dsi_phy_ctrl.lane_byte_clk;//lint !e838
	vfp_time = (uint32_t)inp32(hisifd->mipi_dsi0_base + MIPIDSI_VID_HLINE_TIME_OFFSET) & 0x7fff;//lint !e838
	if (lane_byte_clk != 0) {
	...
	if ((g_dss_version_tag & FB_ACCEL_DSSV320) && (g_fpga_flag == 0)) {
		//get_dsi_phy_ctrl_asic(hisifd, &phy_ctrl);
		get_dsi_phy_ctrl_v320(hisifd, &phy_ctrl, pinfo->mipi.dsi_bit_clk_upt);
	} else {
	...
	if (g_dss_version_tag == FB_ACCEL_DSSV320) {
		...
		set_reg(hisifd->pctrl_base + PERI_CTRL33, 1, 1, 3);
	} else {
