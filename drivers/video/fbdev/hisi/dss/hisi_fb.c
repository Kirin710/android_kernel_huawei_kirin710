...
uint32_t g_mipi_dphy_version = 0;
uint32_t g_mipi_dphy_opt = 0;
...
static void aod_wait_for_offline_blank(struct hisi_fb_data_type *hisifd, int blank_mode)
{
	...
	if (hisifd_list[AUXILIARY_PANEL_IDX] == NULL)
		return;
...
#else
	if (!hisifd_list[PRIMARY_PANEL_IDX]->panel_info.ramless_aod)
		return;
...
static int hisifb_dirty_updt_support_get(struct fb_info *info, void __user *argp)
{
	...
	set_flag_bits_if_cond(hisifd->secure_ctrl.secure_event != DSS_SEC_DISABLE, flag, SECURE_ENABLED_DIRTY_UPDT_DISABLE);
...
static int hisifb_dss_get_platform_product_info(struct hisi_fb_data_type *hisifd, void __user *argp)
{
	...
	get_platform_product_info.dfr_support = hisifd->panel_info.dfr_support;
	...
	HISI_FB_INFO("max_hwc_mmbuf_size=%d, max_mdc_mmbuf_size=%d, fold_display_support=%d,"
		"dummy_pixel_num=%d, dfr_support=%d\n",
		...
		get_platform_product_info.dfr_support);
...
static int hisi_fb_probe(struct platform_device *pdev)
{
	...
	if (!hisi_fb_resource_initialized) {
		...
		hisi_check_and_return(ret,
			-ENXIO, ERR, "failed to get mmbuf_size_max\n");
