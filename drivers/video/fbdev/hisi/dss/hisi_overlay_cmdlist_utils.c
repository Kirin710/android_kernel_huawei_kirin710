	...
	dss_overlay_t *pov_req, uint32_t cmdlist_idxs)
{
	...
	hisi_check_and_no_retval((cmdlist_idxs == 0),
		INFO, "cmdlist_idxs==0");

	mctl_idx = ovl_idx;
	hisi_check_and_no_retval(
		((mctl_idx < 0) || (mctl_idx >= DSS_MCTL_IDX_MAX)),
		ERR, "mctl_idx=%d is invalid", mctl_idx);
	...
	if (g_chip_id == KIRIN710_KEYWEST_CHIP_ID) {
		hisi_mctl_ctl_clear(hisifd, mctl_idx);
#if defined (CONFIG_HISI_FB_V320)
		up(&hisifd->hiace_clear_sem);
#endif
		HISI_FB_INFO("mif_reset clear over");
		return;
	}
