...
static void init_dsc_para(struct dsc_panel_info *vesa_dsc,
...
static void get_porch_clk_para(struct hisi_panel_info *pinfo,
	uint8_t panel_mode)
{
	if (pinfo == NULL) {
	...
	HISI_FB_DEBUG("panel_mode [%d], type[%d], hbp[%d],hfp[%d], hpw[%d]\n",
		panel_mode, pinfo->type,
		pinfo->ldi.h_back_porch,
		pinfo->ldi.h_front_porch,
		pinfo->ldi.h_pulse_width);

	HISI_FB_DEBUG("vbp[%d], vfp[%d], vpw[%d], pxl_clk[%llu], dsi[%d]\n",
...
int switch_panel_mode(struct hisi_fb_data_type *hisifd,
	uint8_t mode_switch_to)
{
	...
	if (ret < 0) {
		HISI_FB_ERR("fb%d pxl_clk_rate %llu failed, error=%d!\n",
	...
	if (mode_switch_to == MODE_10BIT_VIDEO_3X)
		set_reg(hisifd->mipi_dsi0_base + MIPIDSI_MODE_CFG_OFFSET,
			0x0, 1, 0);

	if (is_mipi_cmd_panel(hisifd)) {
		set_reg(hisifd->mipi_dsi0_base + MIPIDSI_CMD_MODE_CFG_OFFSET,
			0x0, 7, 8);
		set_reg(hisifd->mipi_dsi0_base + MIPIDSI_CMD_MODE_CFG_OFFSET,
			0x0, 4, 16);
		set_reg(hisifd->mipi_dsi0_base + MIPIDSI_CMD_MODE_CFG_OFFSET,
...
static int get_jdi_project_id_nt36860c(char *out)
{
	...
	if ((hisifd_primary == NULL) || (out == NULL)) {
		HISI_FB_ERR("NULL pointer\n");
		return -1;
	}

	mipi_dsi_cmds_tx(g_page22_cmd_c, ARRAY_SIZE(g_page22_cmd_c),
		hisifd_primary->mipi_dsi0_base);
	(void)mipi_dsi_lread_reg(g_read_value_c, g_project_id_cmd_c, 10,
		hisifd_primary->mipi_dsi0_base);
	mipi_dsi_cmds_tx(g_page10_cmd_c, ARRAY_SIZE(g_page10_cmd_c),
		hisifd_primary->mipi_dsi0_base);

	/* 10 is len of id cmd, 3 is for byte aligned, 4 is the len of uint32 */
	for (i = 0; i < (10 + 3) / 4; i++)
		HISI_FB_INFO("0x%x\n", g_read_value_c[i]);
...
static int get_project_id_nt36870(char *out)
{
	...
	mipi_dsi_cmds_tx(page21_cmd,
		ARRAY_SIZE(page21_cmd), hisifd_primary->mipi_dsi0_base);
	for (i = 0; i < 10; i++) { /* 10 is len of id cmd */
