...
static int hisi_aod_start_updating_req(aod_display_pos_t *display_pos)
{
	...
	ret = hisi_aod_sendcmd2SensorHub(&pkg_ap, &pkg_mcu, 0); /* 0 means no lock */
#else
	if (is_1bit_ramless_aod_panel())
		ret = hisi_aod_sendcmd2SensorHub(&pkg_ap, &pkg_mcu, 0); /* 0 means no lock */
	else
		ret = hisi_aod_sendcmd2SensorHub(&pkg_ap, &pkg_mcu, 1); /* 1 means lock */
#endif
	if (ret) {
		HISI_AOD_ERR("tag is %d, cmd is %d\n", pkg_ap.tag, pkg_ap.cmd);
		return ret;
...
static int hisi_display_spaces_valid(aod_display_spaces_ap_temp_t spaces_temp)
{
	...
	for (i = 0; i < display_space_count; i++) {
		if (spaces_temp.display_spaces[i].y_size <= 0) {
			HISI_AOD_ERR("invalid parms: x_size=%d, x_start=%d, y_size=%d, y_start=%d\n",
				spaces_temp.display_spaces[i].x_size,
